package com.roger.spider_proxy.fetch.downloader;
...
public class ProxyDownloader implements CloseableDownloader {
...
//    private RedisDao redisDao;
    private RedisDao redisDao=new RedisDao("127.0.0.1",6379);
        ...
        private static final Logger LOGGER = LoggerFactory.getLogger(ProxyDownloader.class);
        ...
        public ProxyDownloader(DownloaderConfig config) {
        ...
        public static ProxyDownloader create(DownloaderConfig config) {
            return new ProxyDownloader(config);
        }
...
//        if(config.getGeneralProxy() != null){
//            requestBuilder.setConfig(RequestConfig.custom().setProxy(config.getGeneralProxy().buildHttpHost()).build());
//        }
            com.roger.spider_proxy.entity.Proxy proxy=redisDao.random();
            if(proxy!=null) {
                requestBuilder.setConfig(RequestConfig.custom().setProxy(new Proxy(proxy.getIp(), proxy.getPort()).buildHttpHost()).build());
            }
            ...
            if (clientConnectionManager == null) {
                throw new IOException("ProxyDownloader has been closed");
            }
                ...
                synchronized (this) {
                    if (clientConnectionManager != null) {
                        try {
                            ...
                            httpClient.close();
                            LOGGER.info("ProxyDownloader has been successfully closed");
                        } catch (IOException e) {
